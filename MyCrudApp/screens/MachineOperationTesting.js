...
const MachineOperationTesting = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`http://192.168.1.15:3000/fetch-operation-variable?vending=${selectedVending}`);
  ...
  const handleUpdate = async () => {
    try {
      ...
      await axios.post(`http://192.168.1.15:3000/update-operation-variable`, { value: editableValue, vending: selectedVending });
      setIsEditing(false);
